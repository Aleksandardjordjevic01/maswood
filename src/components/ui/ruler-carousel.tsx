...
export function RulerCarousel({
  ...
  // totalTrackWidth = n*itemWidth + (n-1)*gap, flex-center puts left edge at -(totalTrackWidth-vw)/2
  // The viewport_center and trackLeft cancel out, leaving a viewport-independent formula:
  // baseOffset = totalTrackWidth/2 - itemsPerSet*step - itemWidth/2
  const itemWidth = 600;
  const gap = 100;
  const step = itemWidth + gap; // 700
  const totalTrackWidth = infiniteItems.length * itemWidth + (infiniteItems.length - 1) * gap;
  const baseOffset = totalTrackWidth / 2 - itemsPerSet * step - itemWidth / 2;
  const targetX = baseOffset - (activeIndex % itemsPerSet) * step;
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center bg-background dark:bg-black py-6">
